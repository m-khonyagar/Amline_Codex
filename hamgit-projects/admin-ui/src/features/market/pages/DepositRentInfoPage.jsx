...
import { PublishAdDialog } from '../components/PublishAdDialog'
import {
  FileStatusOptions,
  MarketRole,
  MarketRoleLabels,
...
export default function DepositRentInfoPage() {
  ...
  const getFileInfo = {
    [MarketRole.LANDLORD]: useGetLandlordFileInfo,
    [MarketRole.TENANT]: useGetTenantFileInfo,
  }[role]

  if (!isValidRole || !isValidId) return <ErrorPage returnUrl="/market/deposit-rent" />

  const depositRentQuery = getFileInfo(id)
  const data = depositRentQuery.data

  return (
    <Page title={`فایل ${MarketRoleLabels[role]}`}>
      <LoadingAndRetry query={depositRentQuery}>
          ...
          <div className="space-y-2.5">
            <div className="flex items-center gap-x-3.5">
              ...
              <Button
                ...
                variant="outline"
                size="sm"
                onClick={() => navigate(`/market/deposit-rent/${role}/${id}/edit`)}
              >
                ویرایش
              </Button>
            </div>
            ...
            <div className="flex items-center gap-x-3">
              <Button className="basis-1/2" size="sm" onClick={() => setIsSendFileDialogOpen(true)}>
