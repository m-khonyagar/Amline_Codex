...
const formSchema = z.object({
  ...
  number_of_owners: z.number().min(1, 'تعداد مالکین باید حداقل ۱ باشد').optional(),
  sale_price: z.number().min(0, 'قیمت نمی‌تواند منفی باشد').optional(),
  ...
  deed_status: z.enum(Object.values(PropertyDeedStatus)).optional(),
  max_tenants: z.number().optional(),
...
export const BuySellSellerForm = ({ id }) => {
  ...
  const create = (_data) => {
    createLandlordFiles.mutate(_data, {
      onSuccess: ({ data }) => {
        toast.success('فایل فروشنده با موفقیت ایجاد شد')
        navigate(`/market/buy-sell/seller/${data.id}`)
      },
  ...
  const update = (_data) => {
    updateLandlordFiles.mutate(_data, {
      onSuccess: () => {
        toast.success('فایل فروشنده با موفقیت ویرایش شد')
        navigate(`/market/buy-sell/seller/${id}`)
      },
      onError: handleErrorOnSubmit,
    })
  }
      ...
      <LoadingAndRetry query={isEditMode ? getLandlordFileQuery : {}}>
        <Form methods={methods} onSubmit={handleSubmit}>
          <div className="grid sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-x-4">
            ...
            <div className="col-span-full text-xl mb-4 mt-8 font-medium">مشخصات فروشنده</div>
            <div>
              <InputField
                label="شماره موبایل"
                name="mobile"
            ...
            <InputNumberField label="ملک چند فروشنده دارد؟" name="number_of_owners" />

            <div className="col-span-full text-xl mb-4 mt-8 font-medium">مبلغ</div>
            <InputNumberField label="قیمت ملک" name="sale_price" suffixIcon={<CheckIcon />} />
            <SelectField
              asValue
              label="نحوه پرداخت"
            ...
            <DatePickerField label="تاریخ تحویل" name="handover_date" />
            <SelectField
              asValue
              multiSelect
              label="انتخاب برچسب"
            ...
            <SelectField
              asValue
              label="فروشنده حاضر به حذف آگهی دیوار بود؟"
              name="monopoly"
              options={MonopolyOptions}
            />
            <InputField label="دلیل عدم حذف آگهی توسط فروشنده" name="reason_for_not_removing_ad" />
