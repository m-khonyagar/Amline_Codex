...
from advertisement.domain.enums import AdType, PropertyAdCategory
...
@router.get("/properties", response_model=List[response_models.PropertyAdResponseModel])
async def get_property_ads(
    params: PaginateParams = Depends(),
    type: AdType | None = Query(None),
    category: PropertyAdCategory | None = Query(None),
    ...
    filters = request_models.GetPropertyAdFilter(
        type=type,
        category=category,
        city=city,
    ...
    return views.get_property_ads(current_user, params, filters, uow)


@router.get("/properties/current-user", response_model=List[response_models.PropertyAdResponseModel])
async def get_user_property_ads(
    current_user: CurrentUser = Depends(di.get_current_user),
    uow: UnitOfWork = Depends(di.get_uow),
):
    return views.get_user_property_ads(uow, current_user)


@router.get("/properties/{id}", response_model=response_models.PropertyAdResponseModel)
async def get_property_ad(
    id: int,
    uow: UnitOfWork = Depends(di.get_uow),
    current_user: User = Depends(di.get_conditional_current_user),
):
    return views.get_property_ad(current_user, id, uow)


@router.post("/properties", response_model=response_models.PropertyAdResponseModel)
async def create_property_ad(
    request_model: request_models.CreatePropertyAdRequest,
    current_user: CurrentUser = Depends(di.get_current_user),
    uow: UnitOfWork = Depends(di.get_uow),
):
    command = dtos.CreatePropertyAdDto(**request_model.model_dump())
    return handlers.create_property_ad_handler(command, uow, current_user)
...
async def delete_property_ad(
    ...
    return OperationResult(success=True, message=expressions_trans.AD_DELETED_SUCCESSFULLY)


@router.post("/report", response_model=OperationResult)
@async_rate_limiter(2, 30)
async def report_an_ad(
    request: Request,
    request_model: request_models.ReportAdRequest,
    uow: UnitOfWork = Depends(di.get_uow),
):
    result = handlers.report_any_ad_handler(
        request_model.ad_id, request_model.ad_category, request_model.description, uow
    )
    return OperationResult(success=result, message=expressions_trans.REPORT_SUBMITTED_SUCCESSFULLY)


@router.post("/visit-requests", response_model=OperationResult)
async def create_visit_request(
    request_model: dtos.CreateVisitRequestDto,
    current_user: CurrentUser = Depends(di.get_current_user),
    uow: UnitOfWork = Depends(di.get_uow),
):
    handlers.create_visit_request_handler(request_model, uow, current_user)
    return OperationResult(success=True, message=expressions_trans.VISIT_REQUEST_SUBMITTED_SUCCESSFULLY)


@router.get("/properties/{id}/similar-ads", response_model=list[response_models.PropertyAdResponseModel])
async def view_similar_property_ads(
    id: int,
    uow: UnitOfWork = Depends(di.get_uow),
    current_user: User = Depends(di.get_conditional_current_user),
):
    return views.get_similar_property_ads(current_user, id, uow)


@router.get("/properties/{id}/similar-wanted-ads", response_model=list[response_models.PropertyWantedAdResponseModel])
async def view_similar_property_wanted_ads_for_an_ad(
    id: int,
    uow: UnitOfWork = Depends(di.get_uow),
    current_user: User = Depends(di.get_conditional_current_user),
):
    return views.get_similar_property_wanted_ads_for_ad(current_user, id, uow)
