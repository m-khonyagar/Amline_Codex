...
def payee_confirmed_receipt_handler(contract_id: int, payment_id: int, user: User, uow: UnitOfWork) -> ContractPayment:
    with uow:
        ...
        payment.status = PaymentStatus.PAYEE_CONFIRMED_RECEIPT
        uow.commit()
